...
export function getInputs():Inputs {
  try {
    const token = coreGetInput('githubToken') || process.env.GITHUB_TOKEN || false
    if (!token) 
      throw new Error(getErrorString('getInputs Error', 500, getInputs.name, 'Received no token, a token is a requirement.'))
    return {
      githubRepo: coreGetInput('githubRepo') || `${context.repo.owner}/${context.repo.repo}`,
      githubToken: token,
      pushBefore: coreGetInput('pushBefore') || ((context.payload.before === undefined) ? false : context.payload.before),
      pushAfter: coreGetInput('pushAfter') || ((context.payload.after === undefined) ? false : context.payload.after),
    ...
    const retVars = Object.fromEntries(Object.entries(process.env).filter( key => key[0].includes('GITHUB') || key[0].includes('INPUT_')))
    throw new Error(getErrorString('getInputs Error', 500, getInputs.name, eString, retVars))
  }
}
/**
 * @function inferInput
 ...
 */
export function inferInput(before: string, after: string, pr: number): Inferred {
  ...
  if (pr) {
    ...
    if (!(before && after)) {
      ...
      throw new Error(getErrorString('inferInput Error', 500, inferInput.name, eString))
    }
