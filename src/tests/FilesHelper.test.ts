  ...
  describe('...with push event...', () => {
    ...
    describe('...with function sortChangedFiles...', () => {
      ...
      ('...correctly sorts GithubFile array into ChangedFiles object %i/10 times', () => {
        ...
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(files, null, 2)))
        const retStats = {files: 0, added: 0, removed: 0, modified: 0} as {[key:string]: number}
      ...
      it('...throws an error', () => {
        expect(() => 
          require('../FilesHelper').sortChangedFiles({filename: '/test/file.txt', status: 'noexist'})).toThrowError(JSON.stringify({
          error: '500/TypeError',
    ...
    describe('...with function writeFiles...', () => {
      it.each(getTestEvents(p.changedFilesInput('push'), 'push'))
      ('...writesFiles %o', (inputName, input, expected) => {
        const coreDebug = require('@actions/core').debug
        const fsWriteFilesSync = require('fs').writeFileSync
        const format = require('../FilesHelper').getFormatExt(inputName.format)
        require('../FilesHelper').writeFiles(inputName.format, 'testKey', input)
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(input, null, 2)))
        expect(fsWriteFilesSync).toHaveBeenCalledWith(`${process.env.HOME}/files_testKey${format}`, expected, 'utf-8')
      })
      it.each(getTestEvents(p.changedFilesInput('push'), 'push'))
      ('...writesFiles %o with files key', (inputName, input, expected) => {
        const coreDebug = require('@actions/core').debug
        const fsWriteFilesSync = require('fs').writeFileSync
        const format = require('../FilesHelper').getFormatExt(inputName.format)
        require('../FilesHelper').writeFiles(inputName.format, 'files', input)
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(input, null, 2)))
        expect(fsWriteFilesSync).toHaveBeenCalledWith(`${process.env.HOME}/files${format}`, expected, 'utf-8')
      })
      it('...throws error', () => {
        const coreDebug = require('@actions/core').debug
        expect(() => 
          require('../FilesHelper').writeFiles('error', 'testKey', 'json')).toThrowError(new Error(JSON.stringify({
          error: '500/TypeError',
          from: 'writeFiles',
          message: 'There was an issue writing output files.',
          payload: JSON.stringify({})}, null, 2)))
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining('Writing output file /Users/bkillian/files_testKey.txt.txt with error and files "json"'))
      })
    })
    /**
    ...
    describe('...with function writeOutput...', () => {
      it.each(getTestEvents(p.changedFilesInput('push'), 'push'))
      ('...writeOutput %o', (inputName, input, expected) => {
        const coreDebug = require('@actions/core').debug
        const coreSetOutput = require('@actions/core').setOutput
        require('../FilesHelper').writeOutput(inputName.format, 'testKey', input)
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(input, null, 2)))
        expect(coreSetOutput).toHaveBeenCalledWith(`files_testKey`, expected)
      })
      it.each(getTestEvents(p.changedFilesInput('push'), 'push'))
      ('...writeOutput %o with files key', (inputName, input, expected) => {
        const coreDebug = require('@actions/core').debug
        const coreSetOutput = require('@actions/core').setOutput
        require('../FilesHelper').writeOutput(inputName.format, 'files', input)
        expect(coreDebug).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(input, null, 2)))
      ...
      it('...throws error', () => {
        const coreDebug = require('@actions/core').debug
        expect(() => 
          require('../FilesHelper').writeOutput('error', 'testKey', 'json')).toThrowError(new Error(JSON.stringify({
          error: '500/TypeError',
          from: 'writeOutput',
          message: 'There was an issue setting action outputs.',
          payload: JSON.stringify({})}, null, 2)))
        expect(coreDebug).toHaveBeenCalledWith('Writing output files_testKey with error and files "json"')
      })
    })
  })
})