  ...
  describe.each(p.testEvents)('...with %s event...', event => {
    ...
    describe('...with function run...', () => {
      describe.each(getTestEvents(p.getFormatExtInputs, 'push'))(
        '...with fileOutput %s...',
        (fileOutputName, fileOutputInput, outputExpected) => {
          describe.each(getTestEvents(p.changedFilesInput(event), event))(
            '...with output %o...',
            (outputInput, outputFiles, expectedOutput) => {
              afterEach(() => {
              ...
              it('...normal', async () => {
                ...
                githubHelper.getChangedFiles = jest.fn(
                  () => getTestFiles(outputFiles).files
                )
                ...
                const processExitMock = jest
                  .spyOn(process, 'exit')
                  .mockImplementation(
                    (code?: number | undefined) => code as never
                  )
