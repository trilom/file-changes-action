  ...
  describe.each(p.testEvents)('...with %s event...', event => {
    ...
    describe('...with function getInputs...', () => {
      it('...sets correct default input parameters.', () => {
        ...
        const { prNumber, pushAfter, pushBefore, githubToken, githubRepo, output, fileOutput, event:inputEventName } = require('../InputHelper').getInputs()
      ...
      ('...%s', (title, input, expected) => {
        ...
        if (title.includes('ERROR with no')) {
          ...
          expect(() => {
            ...
            error: '500/inferInput Error',
            from: 'inferInput',
            message: `Received event from ${eventName(event)}, but only received a before(${input.before}) or after(${input.after}).\n I need both of these if you want to use a Push event.`,
            payload: ""
          }, null, 2)))
        } else {
