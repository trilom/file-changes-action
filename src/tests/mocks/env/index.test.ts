...
describe.each(p.testEvents)
('Testing Env object with %s event...', (event) => {
  beforeAll(() => { env = new Env({}, {}, event) })
  it('...Env can get correct test file inputs', () => {
    const { files, stats } = getTestFiles()
    const filesCount = Object.values(stats).reduce((prev, curr) => prev + curr)/2
  ...
  it('...Env can get custom files', () => {
    const { files, stats } = getTestFiles(['/test/test', '/test1/test1', '/test2/test2', '/test3/test3'])
    const filesCount = Object.values(stats).reduce((prev, curr) => prev + curr)/2
  ...
  it('...Env can get custom file properties', () => {
    const { files, stats } = getTestFiles(['/test/test', '/test1/test1', '/test2/test2', '/test3/test3'], {files: 0, test: 0, test1: 0, test2: 0, test3:0 })
    const filesCount = Object.values(stats).reduce((prev, curr) => prev + curr)/2
    expect(files.length).toBe(4)
    expect(filesCount).toBe(4)
    expect(stats.files).toStrictEqual(4)
  ...
  it('...Env can get custom files limited', () => {
    const { files, stats } = getTestFiles(['/test/test', '/test1/test1', '/test2/test2', '/test3/test3', '/test4/test4', '/test5/test5', '/test6/test6', '/test7/test7'], { files: 0, added: 0, removed: 0, modified: 0 }, 4)
    const filesCount = Object.values(stats).reduce((prev, curr) => prev + curr)/2
    expect(files.length).toBe(4)
  ...
  it('...Env can get correct process.env.GITHUB_EVENT_NAME', () => {
    ...
    else if (result.includes('pull_request')) expect(result).toBe('pull_request')
    else if (result.includes('issue_comment')) expect(result).toBe('issue_comment')
  ...
  it('...Env can format strings for action process.env.INPUT_INPUTNAME', () => {
    ...
    expect(result).toStrictEqual({
      INPUT_TEST: 'value', INPUT_REALTEST: 'realVALUE', 
  ...
  if (event === 'push') {
    ...
    it('...Env mocks Octokit.repos.paginate for pr with custom callback', async () => {
      const request = p.OctokitPaginatePrRequest
      const response = p.OctokitPaginatePrResponse
      return env.octokitMock.paginate(request, res => {
        return res.data
      }).then(data => {
        expect(data).toStrictEqual(response)
        return expect(data.length).toBe(7)
