  ...
  describe('...with push event...', () => {
    ...
    describe('...with function getErrorString...', () => {
      ...
      it('...can throw an error for my error', () => {
        ...
        obj.a = {b:obj}
        expect(() =>
          require('../UtilsHelper').getErrorString('test', 200, 'test', 'test', obj)
        ).toThrowError(JSON.stringify({name: '500/undefined', message: 'Error throwing error.'}))
        // expect(JSON.stringify(JSON.parse(error))).toBe(JSON.stringify({error:'500/undefined', payload:''}))
        expect(setFailed).toBeCalledWith(expect.stringContaining('Error throwing error.'))
        expect(coreError).toBeCalledWith(expect.stringContaining('Error throwing error.'))
      })
    })
    /**
    ...
    describe('...with function errorMessage...', () => {
      it.each(getTestEvents(p.errorMessageInputs, 'push'))('...for function %s', (f, e, expected) => {
